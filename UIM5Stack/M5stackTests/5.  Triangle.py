...
#btn_indoor = M5Btn('', x=5, y=40, w=100, h=140, bg_c=0x90EE90, text_c=0x000000, parent=None)
#btn_outdoor = M5Btn('', x=115, y=40, w=100, h=140, bg_c=0x87CEEB,text_c=0x000000, parent=None)
btn_co2 = M5Btn('', x=220, y=40, w=100, h=140, bg_c=0xFFD700,text_c=0x000000, parent=None)


# ---------- Ligne 2 : Boutons + Icônes (Indoor / Outdoor / CO2) ----------
btn_width = 90
btn_height = 60
btn_y = 60
btn_spacing = 15
...
# Indoor
btn_indoor = M5Btn("", x=15, y=btn_y, w=btn_width, h=btn_height, bg_c=0x222222)
btn_indoor.set_btn_text("22°C\n400ppm")
btn_indoor.set_btn_text_color(0xFFFFFF)
btn_indoor.set_btn_text_font(lcd.FONT_DejaVu18)
icon_indoor = M5Img(15 + (btn_width // 2) - 16, icon_y, "res/thermometer.png")

# Outdoor
btn_outdoor = M5Btn("", x=15 + btn_width + btn_spacing, y=btn_y, w=btn_width, h=btn_height, bg_c=0x222222)
btn_outdoor.set_btn_text("18°C\n500ppm")
btn_outdoor.set_btn_text_color(0xFFFFFF)
btn_outdoor.set_btn_text_font(lcd.FONT_DejaVu18)
icon_outdoor = M5Img(15 + btn_width + btn_spacing + (btn_width // 2) - 16, icon_y, "res/weather.png")




# Labels des valeurs (toujours sans parent)
label_indoor = M5Label('-- °C\n-- %', x=55, y=110, color=0x000000, font=FONT_MONT_14)
label_outdoor = M5Label('-- °C', x=165, y=120, color=0x000000, font=FONT_MONT_14)
label_co2 = M5Label('-- ppm', x=270, y=120, color=0x000000, font=FONT_MONT_14)

label_debug = M5Label('', x=5, y=200, color=0x0000, font=FONT_MONT_14, parent=None)  # label pour afficher erreurs
label_hello = M5Label('', x=220, y=10, color=0xFF0000, font=FONT_MONT_18, parent=None)




label_indoor.set_align(ALIGN_CENTER, 0, 10)
label_outdoor.set_align(ALIGN_CENTER, 0, 10)
label_co2.set_align(ALIGN_CENTER, 0, 10)
...
label_debug = M5Label('', x=5, y=200, color=0x0000, font=FONT_MONT_14, parent=None)  # label pour afficher erreurs
label_hello = M5Label('', x=220, y=10, color=0xFF0000, font=FONT_MONT_18, parent=None)
# Charger l'image (doit être dans les assets)
img = M5Img("res/weatherIcon_imresizer.png", x=0, y=0)
img2 = M5Img("res/home_imresizer.png", x=30, y=30)
...
def read_outdoor_temp():
    try:
        url = 'http://api.openweathermap.org/data/2.5/weather?q={},{}&appid={}&units=metric'.format(CITY, COUNTRY,
                                                                                                    API_KEY)
...
def read_forecast_week():
    try:
        url = 'http://api.openweathermap.org/data/2.5/forecast?q={},{}&appid={}&units=metric'.format(CITY, COUNTRY,
                                                                                                     API_KEY)
        res = urequests.get(url)
        data = res.json()
        res.close()
        label_debug.set_text("aie" + str(data))
        daily_forecast = {}
...
def send_to_bigquery(label_debug, temp, humidity):
